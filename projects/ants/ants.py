...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        #We want Place to keep track of entrances! 1 entrance / Place
        if self.exit != None: 
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    if isinstance(self.ant, QueenAnt) and not self.ant.is_imposter:
                        bees_win()
                    else:
                        self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if isinstance(self.ant.ant, QueenAnt) and not self.ant.ant.is_imposter:
                        has_queen = True
                    else:
                        self.ant.ant = None
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        #ninja ants can still be in the place of the bee, so we AND check if the ant block_path or no
        # END Problem 8
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    min_range = 0 #closest bee that can be hit
    max_range = float('inf') #farthest bee that can be hit with a SAFE upper bound buffer
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5 go from square to square looking for a bee in bees
        squares_away = 0 #from home square
        curr_target_place = self.place #add .bees to get the list of the bees there

        if self.min_range <= squares_away:
            target_bee = random_or_none(curr_target_place.bees)
        else:
            target_bee = None
        while not target_bee: #while not None
            if curr_target_place.entrance != hive:
                curr_target_place = curr_target_place.entrance
                squares_away += 1
                if self.min_range <= squares_away and self.max_range >= squares_away:
                    target_bee = random_or_none(curr_target_place.bees)
            else:
                break 
        return target_bee #either None OR a random bee in the square
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3
        Place.add_insect(self, insect) #need self because called on the CLASS- need both arguments
        if not insect.watersafe:
            insect.reduce_armor(insect.armor) #no Class being called, insect goes in as first argument
        # END Problem 3
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            the_bees = self.place.bees[:]
            for bee in the_bees:
                bee.reduce_armor(self.damage)
        ANT.reduce_armor(self, amount)
        #this last line to the end to kill the ant AFTER the bees will die
        # END Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    food_cost = 2
    # BEGIN Problem 6
    #throw past the closer bee to the farther one
    implemented = True   # Change to True to view in the GUI
    # END Problem 6
...
class NinjaAnt(Ant):
    ...
    #for doctest, there were 3 bees in range, so we strike 3 of them -1 damage each
    name = 'Ninja'
    damage = 1
    # BEGIN Problem 8
    food_cost = 5
...
#container and contained need to both occupy the same square
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 11
        self.ant = ant #now we contian an ant
        # END Problem 11
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 13
        Ant.__init__(self)
        self.is_imposter = not self.true_ant_queen
        QueenAnt.true_ant_queen = False
        self.ants_behind = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            square = self.place.exit
            while square != None:
                if square.ant != None:
                    if square.ant not in self.ants_behind:
                        self.ants_behind.append(square.ant)
                        square.ant.damage *= 2
                    if square.ant.container == True or isinstance(square.ant, BodyguardAnt):
                        if square.ant.ant != None and square.ant.ant not in self.ants_behind:
                            self.ants_behind.append(square.ant.ant)
                            square.ant.ant.damage *= 2
                square = square.exit
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    # effect is make_slow or make_stun
    # make a new action method for the bee based on the effect for the duration
