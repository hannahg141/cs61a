...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        elif person in self.place.characters: #self to distinguish what place i am in
            message = self.place.characters[person].talk() #or character.talk(person)
            #self.place.characters is my dictionary of all the characters in the place of me
    ...
    def take(self, thing):
        ...
        1
        """
        #to put things in my backpack
        #backpack = []

        #thing is the name of the object
        #does the place have a thing?
        if type(thing) != str:
            print('Thing should be a string.')
        else:
            if thing not in self.place.things: #is the thing in this list of things
                print(thing + ' is not here.')
            else:
                item = self.place.take(thing)
                self.backpack.append(item)
                print(self.name + ' takes the ' + str(item.name))
            
       

        #takes an object name as name, checks if Place has the Thing 
        #then puts it in my backpack
        #things instance in Place is a dictionary mapping Thankg Names (strings) : Thing objects
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            ...
            self.place = self.place.get_neighbor(place)
            print(str(self.place.name) + " can't be unlocked without a key!")
