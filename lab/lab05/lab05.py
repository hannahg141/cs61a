    ...
    i_love_music
      ...
      trance
        darude
          sandstorm
    """
    left = tree('pop', [tree('justin bieber', [tree('single', [tree('what do you mean?')])]), tree('2015 pop mashup')])
    right = tree('trance', [tree('darude', [tree('sandstorm')])])
    return tree(username, [left, right])
...
def find(t, target):
    ...
    if root(t) == target:
        return True
    for word in branches(t):
        if find(word, target) == True:
