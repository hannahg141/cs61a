...
#evaluates expressions to get values
class Expr:
    """
...
#always evaluate to a number value
class Literal(Expr):
...
class Name(Expr):
    ...
    #takes and environment as an argument! 
        #we need to know where we are when evaluating the name
    #env: a dictionary name(str): value(instance of value class)
    #return the right value bound to the name in the CURRENT ENVIRONMENT
    def eval(self, env):
        """
        >>> env = {
        ...     'a': Number(1),
...
#function is a result of evaluating an expression
...
#self.operator(an expression) and self.operands
class CallExpr(Expr):
    ...
    def __init__(self, operator, operands):
        ...
        self.operands = operands #a list of operands - might be a lot
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        operator = Name.eval(self.operator, env) #evaluate the operator
        all_operands = [] 

        for operand in self.operands: 
            if isinstance(operand, Literal):
                all_operands += [Literal.eval(operand, env)]
            elif isinstance(operand, Name):
                all_operands += [Name.eval(operand, env)]
            elif isinstance(operand, LambdaExpr):
                all_operands += [LambdExpr.eval(operand, env)]
            elif isinstance(operand, CallExpr):
                all_operands += [CallExpr.eval(operand, env)]
            else:
                raise TypeError('sorry, try again with valid operands')
        return operator.apply(all_operands)
