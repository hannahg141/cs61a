...
def summation_using_accumulate(n, term):
    ...
    if n == 0:
        ...
        return term(n) + accumulate(add, base, n-1, term)
...
def product_using_accumulate(n, term):
    ...
    if n == 1:
        ...
        #print(n)
        return term(n) * accumulate(mul, base, n-1, term)
...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    # while i <= n:
    #     if term(i) 
    #     total = combiner_if(total, term(i))
    #     i += 1
    # return total

    # x = base y = current number
    def combine_if(x, y): #instead of combiner to FILTER the results of term (only odd / even, prime, etc)
        if pred(y):
            #print(y)
            #total += combiner(total, term(y))
            return combiner(x, y)
        else:
            return x
